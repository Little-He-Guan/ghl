...
DEFINE_TEST_CASE(test_binary_search_tree_insert)
	...
	// insert into an empty tree
	{
		...
		ASSERT_FALSE(bst.get_root()->empty(), "expected to have the element inserted")
		ASSERT_EQUALS(6, bst.get_root()->get_obj(), "expected to have the inserted element correct")
		ASSERT_TRUE(bst.get_root()->left_empty() && bst.get_root()->right_empty(), "expected to not modify the branches")
	}
	...
	// insert into a non-empty tree
	{
		...
		ASSERT_FALSE(bst.get_root()->left_empty(), "it should be inserted onto left")
		ASSERT_EQUALS(1, bst.get_root()->left()->get_obj(), "expected to get the value correct")

		// insert a one bigger than min and smaller than root
		bst.insert(data_to_be_inserted[2 - 1]);
		ASSERT_FALSE(bst.get_root()->left()->right_empty(), "it should be inserted onto the right of 1")
		ASSERT_EQUALS(2, bst.get_root()->left()->right()->get_obj(), "expected to get the value correct")

		// insert a one bigger than root
		bst.insert(data_to_be_inserted[10 - 1]);
		ASSERT_FALSE(bst.get_root()->right_empty(), "it should be inserted onto right")
		ASSERT_EQUALS(10, bst.get_root()->right()->get_obj(), "expected to get the value correct")

		// insert a one smaller than max and bigger than root
		bst.insert(data_to_be_inserted[9 - 1]);
		ASSERT_FALSE(bst.get_root()->right()->left_empty(), "it should be inserted onto the left of 10")
		ASSERT_EQUALS(9, bst.get_root()->right()->left()->get_obj(), "expected to get the value correct")
		...
		// set node to 2
		auto* node = bst.get_root()->left()->right();
		ASSERT_FALSE(node->right_empty(), "5 should be inserted onto the right of node")
		ASSERT_EQUALS(5, node->right()->get_obj(), "expected to get the value correct")
		node = node->right();
		ASSERT_FALSE(node->left_empty(), "4 should be inserted onto the left of node")
		ASSERT_EQUALS(4, node->left()->get_obj(), "expected to get the value correct")
		node = node->left();
		ASSERT_FALSE(node->left_empty(), "3 should be inserted onto the left of node")
		...
		// set node to 9
		node = bst.get_root()->right()->left();
		ASSERT_FALSE(node->left_empty(), "7 should be inserted onto the left of node")
		ASSERT_EQUALS(7, node->left()->get_obj(), "expected to get the value correct")
		node = node->left();
		ASSERT_FALSE(node->right_empty(), "8 should be inserted onto the right of node")
...
DEFINE_TEST_CASE(test_binary_search_tree_remove)
	...
	// try to remove something who has no children (then it will become nullptr simply)
	{
		...
		ASSERT_TRUE(iter->right_empty(), "expected to have its right nullptr (10 successfully removed)")
	}
	...
	// try to remove something who has one child (then that child will take its place)
	{
		...
		auto * p3 = bst.get_root()->left();
		ASSERT_FALSE(p3->left_empty(), "its place should be taken")
	...
	// no rearrangement (when its successor is one of its children)
	{
		...
		auto * p9 = bst.get_root()->right();
		ASSERT_EQUALS(9, p9->get_obj(), "its child should take its place")
		ASSERT_FALSE(p9->left_empty(), "its another child should not be modified")
	...
	// rearrangement occurs (when its successor is not one of its direct children)
	{
		...
		ASSERT_FALSE(bst.get_root()->right()->left_empty(), "7's original right child should be rearranged")
		ASSERT_EQUALS(8, bst.get_root()->right()->left()->get_obj(), "7's original right child should be modified")
	}
